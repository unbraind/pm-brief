 ...
 *
 * Every shipped script (the coverage gate, the docstring gate, the merge-driver
 * preparer and the publish-attestation verifier) must behave identically when
 * imported by their suites versus executed as `main`, so the comparison lives
 * in exactly one measured module and can never drift between copies.
 */
